	...
	auto ins = [&](string &s, int decide, int satnode) {
		...
		for(char c : s) {
			...
			if(trie[curr].ch[val] == -1) {
				...
				trie.emplace_back();
				trie.back().s = trie[curr].s;
				trie.back().s.push_back(val + '0');
			}
	...
	for(Node p : trie) {
		...
		if(k != 0) {
			...
			*/
			...
			for(int i = 0; i < k; ++i) {
				cerr << p.satnode << " -> " << neg(p.end[i]) << endl;
				// implies(p.satnode, neg(p.end[i]));
				// addor(neg(p.satnode), neg(p.end[i]));
				// addor(p.satnode, p.satnode);
				// addor(neg(p.satnode), neg(p.satnode));
				// addor(neg(p.satnode), neg(p.end[i]));
			}
			global_sat += 2*d;
		}
	...
	*/
	...
	for(Node p : trie) {
		for(int i = 0; i < p.end.size(); ++i) {
			cerr << p.satnode << " ~ " << p.end[i] << endl;
			// addor(neg(p.satnode), neg(p.end[i]));
		}
	}
	*/

	cerr << "GLOB " << global_sat << endl;
	addor(neg(8), neg(2));
	addor(neg(9), neg(3));
	addor(neg(10), neg(4));
	addor(neg(13), neg(5));
	addor(8, neg(3));
	bool b = satisf(10000);
	cerr << "BBBBBBBBBB " << b << endl;
	assert(b);
	assert(truth[8] || truth[neg(3)]);
	assert(truth[neg(8)] || truth[neg(2)]);

	for(Node p : trie) {
		cerr << p.s << " " << p.satnode << " " << truth[p.satnode] << endl;
		for(int i = 0; i < p.end.size(); ++i) {
			cerr << neg(p.satnode) << " = " << neg(p.end[i]) << endl;
			assert(truth[neg(p.satnode)] || truth[neg(p.end[i])]);
			cerr << p.end[i] << " = " << truth[p.end[i]] << endl;
		}
		cerr << endl;
	}
	if(!b) {
		...
		cout << "YES\n";
		for(int i = 0; i < ene; ++i) {
			for(char c : ns[i]) if(c == '?') cout << !truth[sats[i]];
			else cout << c;
			cout << "\n";
			// cout << truth[sats[i]] << " ";
		}
