...
void dfs(int node, int len, vector<int> &dist, vector<vector<Conn>> &tree) {
...
vector<int> calc_dist(int node, vector<vector<Conn>> &tree) {
...
int main() {
    ...
    int diam = dist_from_fst[snd];
    cout << fixed;
    cout << setprecision(1);
    cout << (double) diam / 2.0 << endl;
